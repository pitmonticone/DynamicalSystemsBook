...
Before we get to this abstract definition, we will take our time exploring the
...
and that $\cat{A}(\id_D) = \id_{\cat{A}(D)}$.\footnote{Here, we are hiding some
  ...
  to a coherence isomorphism. Again, the issue involves shuffling parentheses
...
\item (Unitality) The profunctor $\cat{A}(\id_{D_1}) : \cat{A}(D_1) \tickar
  ...
  $\cat{A}(f)$ on the left and right respectively. We may summarize this may
  saying that 
...
\begin{itemize}
  ...
  \item For every square
    ...
    \begin{itemize}
    \item $F$ commutes with horizontal composition: $F(\alpha \mid \beta) = F\alpha \mid F\beta$.
    \item $F$ commutes with vertical composition: $F\left( \frac{\alpha}{\beta} \right) = \frac{F\alpha}{F\beta}$.
...
composing horizontally second, we arrive at $(F \then G)^{\frac{j}{k}}$, while if
...
ergodic theory we most often use the category of measurable spaces and
measurable functions.\footnote{We most often consider maps which preserve a
  ...
  needs of a subject.} We often assume the dynamics of the systems are not arbitrary
...
We therefore arrive at the following formal definition of change of systems theory.
...
update $\phi_{\ast}\update{S}(s, i)$ of state $s$ with input $i$ just when just
...
\]
from the deterministic systems theory in $\cat{C}$ to the cartesian systems theory in $\cat{D}$.
\end{proposition}
\begin{proof}
...
dynamical system without the constraint of continuity, simply by forgetting that
...
An example of this is the Euler approximation, which takes a Euclidean
...
$\varphi$ and $\lens{f_{\flat}}{f}$ in the appropriate double category of arenas, and here
  ...
  which sorts of behaviors are preserved even in situations where it may not be
...
the Runge–Kutta approximation can be seen as a change of systems theory; see \cite{ngotiaoco2017compositionality}.
