...
\chapter{Non-deterministic systems theories}\label{sec.non_deterministic_systems}
...
systems theories. The kind of non-determinism --- possibilistic, stochastic, etc.
...
\begin{remark}
  While \cref{def.deterministic_system} can be interpreted in any cartesian
...
few terms. First of all, since $\eta_M = \id_{MX}$, if $m$ is an element of $MX$, then
\[
  \unalignedDo{
    $x \from m$,
    $\eta(x)$
...
probability of a given change occurring, conditioned upon the current state. A
...
sample wouldn't give us very useful statistics, but if we use the combined works
...
\begin{definition}\label{def.cost_monad}
  ...
  assigning the best-case cost of producing a $b \in B$ from a given $a \in A$.
...
\Fun{Cost}(A) \coloneqq \{c : A \to [0, \infty] \mid \mbox{$\{a \in A \mid c(a) <
  ...
  \item For a set $A$, $\eta_{\Fun{Cost}} : A \to \Fun{Cost}(A)$ assumes that we
    can only produce what we have, but that if we already have it, it's free. Formally:
\[
\eta_{\Fun{Cost}}(a)(a') \coloneqq \begin{cases} 0 &\mbox{if $a = a'$} \\
...
\begin{exercise}
  Show that the monad of $R$-valued rewards is really a commutative monad.
...
We will show that the assignment $A \mapsto \probset(\rr \times A)$
 ...
 \[
\lambda \coloneqq R \times MA \xto{\eta_M \times \id} MR \times MA \xto{\sigma^M} M(R \times A)
 \]
Intuitively, this takes a reward $r \in R$ and a non-deterministic $a \in MA$
and gives us the non-deterministic pair $(r, a)$.
...
\end{proposition}
\begin{proof}
It is not obvious that this will satisfy the monad laws, but it is a rather
...
In the same way that $0$ is a number, deterministic systems are 
...
\begin{exercise}
Show that $\probset \eta_{\rr} : \probset A \to \probset(\rr \times A)$ is a
commutative monad morphism. That is, show that the following diagrams commute:
\begin{enumerate}
\item
  \begin{equation}
    \begin{tikzcd}
    A \ar[r, "\eta_{\probset}"] \ar[dr, "\eta_{\probset(\rr \times -)}"'] & \probset A \ar[d, "\probset(\eta_{\rr})"] \\
    & \probset(\rr \times A) 
    \end{tikzcd}
  \end{equation}
\item
...
\cref{eqn.com_monad_map_lift} commutes for all $f : A \to M B$, we may take $f
...
\begin{proposition}
  Let $\phi : M \to N$ be a commutative monad morphisms. Then there is a
...
the signal generated by our source and generates their own signal in response:
  \[
\begin{tikzpicture}[oriented WD, every fit/.style={inner xsep=\bbx, inner ysep=\bby}, bbx = 1cm, bby =.5cm, bb min width=1cm, bb port length=4pt, bb port sep=1, baseline=(X.center)]
...
This construction works for \emph{any} indexed category $\cat{A} : \cat{C}\op \to
...
definition). We will interpret the base category $\cat{C}$ as the category of
...
\cite{spivak2019generalized}. This generalization of lens will underlie our
